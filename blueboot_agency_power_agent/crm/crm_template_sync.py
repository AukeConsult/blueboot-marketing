"""
crm_template_sync.py -- CLI wrapper for crm_template_sync_lib.

Logic lives in functions-crm/crm/crm_template_sync_lib.py.
...
from crm.crm_template_sync_lib import run_template_sync, run_template_enrich
...
def main():
    ...
    if args.enrich:
        count = run_template_enrich(db=db, svc=svc, tab=args.tab)
        print(f"[crm-template] Enrich done -- {count} items matched.")
    else:
        count = run_template_sync(db=db, svc=svc, tab=args.tab)
        print(f"[crm-template] Done -- {count} docs synced.")
