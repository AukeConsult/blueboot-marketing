"""leads_email_check.py — Classify contacts by email type and contact role.

Reads contacts that have an email address (skipping those already checked),
...
Usage:
    ...
    python app\\leads_email_check.py --countries UK --batch-size 50 --concurrent 4
"""
...
def _load_secrets():
    ...
    try:
        ...
        print(f"  [leads-email-check] secrets error: {e}")
        return None, None
...
# travel over the wire until a contact is matched to a ready (AI-classified) lead.
_CONTACT_MATCH_FIELDS = ["email", "country", "ai_country",
                         "social_enriched_at", "email_checked_at"]
_LEAD_MATCH_FIELDS    = ["ai_classified_at"]
...
def _scan_one_partition(query, countries, force) -> list:
    ...
    for doc in query.stream():
        ...
        if not (data.get("email") or "").strip():
            continue
        if not (data.get("social_enriched_at") or "").strip():
            continue                                   # lead_enrich_contacts not done -> not ready
...
def _parallel_get_all(db, refs: list, field_paths, workers: int, label: str) -> list:
    ...
    with _futures.ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in _futures.as_completed(futs):
            ...
            print(f"  [ready] {label} {done:>6,}/{total:,}", flush=True)
    return snaps
...
def _load_contacts(db, countries: list[str] | None, limit: int | None,
    ...
    """Load lead contacts READY for email-check but NOT yet email-checked.

    Ready = prior enrichment stages complete:
        contact doc : social_enriched_at (lead_enrich_contacts)
        parent lead : ai_classified_at   (lead_enrich_agent)
    ...
    Phase 1 (minimal reads) - scan contacts in parallel partitions reading only
      marker fields, batch-read parent leads' markers, then match.
    ...
    print(f"  [ready] Phase 1a - scanning contacts in up to {scan_partitions} "
          f"parallel partitions (markers only)...", flush=True)
    try:
        partitions = list(cg.get_partitions(scan_partitions))
        queries    = [pt.query().select(_CONTACT_MATCH_FIELDS) for pt in partitions]
    except Exception as exc:
        print(f"  [ready] partitioning unavailable ({exc}); single-pass scan", flush=True)
    ...
    with _futures.ThreadPoolExecutor(max_workers=scan_workers) as ex:
        ...
        for fut in _futures.as_completed(futs):
            ...
            done_parts += 1
            print(f"  [ready] ...partition {done_parts}/{len(queries)} done  "
                  f"contact-matched so far {len(cand_refs):>6,}", flush=True)
    print(f"  [ready] Phase 1a done - {len(cand_refs):,} contacts pass contact checks",
          flush=True)
    if not cand_refs:
        return []

    # ---- Phase 1b: parallel parent-lead marker read (deduped) ----
    parent_by_path = {r.parent.parent.path: r.parent.parent for r in cand_refs}
    lead_refs = list(parent_by_path.values())
    print(f"  [ready] Phase 1b - checking {len(lead_refs):,} unique parent leads...",
          flush=True)
    ready_leads: set = set()
    for snap in _parallel_get_all(db, lead_refs, _LEAD_MATCH_FIELDS,
                                  read_workers, "...leads checked"):
        if not snap.exists:
            continue
        d = snap.to_dict() or {}
        if (d.get("ai_classified_at") or "").strip():
            ready_leads.add(snap.reference.path)
    print(f"  [ready] Phase 1b done - {len(ready_leads):,} leads AI-classified",
          flush=True)

    # ---- Phase 1c: match contacts to ready leads ----
    matched_refs = [r for r in cand_refs if r.parent.parent.path in ready_leads]
    if limit:
        matched_refs = matched_refs[:limit]
    print(f"  [ready] Phase 1c - {len(matched_refs):,} contacts matched to ready leads",
          flush=True)
    if not matched_refs:
        return []

    # ---- Phase 2: parallel full-detail load for matched contacts ----
    print(f"  [ready] Phase 2 - loading full detail for {len(matched_refs):,} contacts...",
          flush=True)
    results: list = []
    ...
    print(f"  [ready] Done - {len(results):,} ready contacts loaded", flush=True)
    return results
...
async def _enrich_batch(client, loop, batch_refs, batch_data, counters, dry_run):
    ...
    for ref, data in zip(batch_refs, batch_data):
        ...
        try:
            ...
            print(f"    [leads-email-check] write error for {domain}: {e}")
            counters["failed"] += 1
...
async def _run_async(client, to_process, batch_size, concurrent, dry_run):
    ...
    print(f"  [leads-email-check] {len(to_process)} contacts  ->  {total_b} batches of <={batch_size}"
    ...
    try:
        ...
        print(f"  [leads-email-check] OVERALL TIMEOUT — cancelling", flush=True)
        for t in tasks:
            t.cancel()
        await asyncio.gather(*tasks, return_exceptions=True)

    print(f"\n  [leads-email-check] Done.  updated={counters['updated']}  "
...
def main(argv=None):
    p = argparse.ArgumentParser(description="Classify contacts by email type and contact role")
    ...
    if not to_proc:
        print("  [leads-email-check] Nothing to classify.")
        return
