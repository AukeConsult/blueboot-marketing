...
"""
Adapted copy of app/mail_reader.py. Identical logic; only the two imports
are relative (`.firestore_client` / `.mail_accounts_config`).
"""
import imaplib
import email
...
def read_unread_emails(account: str | None = None):
    """Poll one mailbox's INBOX for unread messages and store them in `inbox_messages` (deduped by Message-ID)."""
    ...
    if not imap_host:
        raise RuntimeError(
            ...
            f"-- set {acc['alias'].upper()}_IMAP_HOST / _IMAP_PORT in .env.<project-id>"
        )
