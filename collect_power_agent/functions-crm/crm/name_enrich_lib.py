...
"""name_enrich_lib.py -- Name enrichment core logic for use by crmWorker.

Imported by functions-crm/main.py (Cloud Tasks worker).
CLI entry point lives in app/campaign_name_enrich.py.
"""
...
def _get_db():
    """Stub — always pass db= explicitly when calling from crmWorker."""
    raise RuntimeError("Pass db= explicitly; _get_db() not available in crm deployment")
...
async def _brave_search_name(session, email: str, domain: str, local: str) -> dict:
    ...
    """
    import os as _os
    api_key = _os.getenv("BRAVE_API_KEY", "").strip()
    if not api_key:
