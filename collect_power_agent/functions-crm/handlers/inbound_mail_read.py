"""handlers/inbound_mail_read.py - inbound mail read job trigger."""
...
bp = Blueprint("inbound_mail_read", __name__)
...
@bp.route("/api/crm/inbound-mail-read", methods=["POST"])
def inbound_mail_read():
    ...
    try:
        ...
        job_id = _new_job("inbound-mail-read", params)
        _enqueue_task("inbound-mail-read", job_id, params)
        return _accepted(job_id, "inbound-mail-read")
